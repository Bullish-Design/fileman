# CONCEPT — `lsd-json` MVP (Tree-sitter Grammar Development)
...
2. **lsd-json:** Python uv wrapper script loading compiled grammar
...
def extract_lsd_data(directory: Path) -> list[dict]:
    """Run lsd -> lsd-json pipeline, extract fields"""
...
```nix
{
  tasks = {
    ...
    "test:manual" = {
      exec = ''
        lsd --long test_files | wrappers/lsd-json --pretty --include-text
      '';
      after = ["build:wrapper", "test:generate-files"];
    };
  };
...
```nix
{
  scripts = {
    ...
    # Convenience wrapper
    lsd-parse.exec = ''
      lsd --long "$@" | lsd-json --pretty --include-text
    '';
...
- [ ] `lsd-json` loads compiled grammar successfully
...
### Integration
- [ ] `lsd --long /path | lsd-json --pretty` produces readable output
- [ ] devenv.nix exposes `lsd-json` command globally in shell
...
- `lsd-json run --` mode (execute lsd internally)
...
### Productionize for kubectl parsing
1. Create tree-sitter-kubectl grammar for table output
2. Handle kubectl column headers
3. Extract pod status, restarts, age fields
